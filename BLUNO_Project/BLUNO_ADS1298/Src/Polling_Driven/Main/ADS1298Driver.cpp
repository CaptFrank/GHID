...
/**
 * This method receives a response from the ADS1298
 *
 * @return response 				- The response
 */
uint8_t* ADS1298_Driver::receive_response(){
	
	//! We cycle through the receive structure
	for(register uint8_t i = 0; i < sizeof(rx_buffer_t); i ++) {
		
		//! We get the bytes
		((uint8_t*)&this->_rx_buff)[i] = SPI.transfer(0);
		delayMicroseconds(5);
	}
}
...
void ADS1298_Driver::check_active_channels(){
	...
	uint8_t temp;
	temp = this->_configs._active;
	temp = this->_configs._channels;
...
void ADS1298_Driver::_init_pins(){
	...
	//! We setup the pins
	//! Delay the pin power up sequence
	delay(1000);
