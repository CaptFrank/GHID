...
spi_settings_t spi_settings = {
						...
						MSBFIRST,				//! msb first
						SPI_CLOCK_DIV16,		//! devide speed by 16 (16M/16)
...
//! The CC2540 Driver
//! 	- Here we use the default setup function.... We could change it
//!		- Only comment this line out if you need to modify the original BLUNO AT command settings.
//CC2540_Driver cc2540_driver((char*)"ADS1298", (char*)command_pointers, &dispatcher);

//! The connection
Bluetooth_Connection_Handler connection(&Serial, DATA_STREAM_BASED, &buffer, 
...
// 	digitalWrite(5, HIGH);
	...
	//! We connect to the host device - We don't need this for our ECG app
	//connection.connect();
...
// 	uint8_t* response = ads1298_driver.receive_response();
// 	
// 	Serial.println("--------------------");
...
// 	}
// 	Serial.println("--------------------");
// 	Serial.println(DATA_PACKET_SIZE);
// 	Serial.println(RingBuffer_GetCount(ads1298_driver._buff));
// 	Serial.println("--------------------");
