...
typedef struct sensor_packet_data_t {
	...
	struct sensor_config_t {
		...
		static const uint8_t 		packet_sensor_axis = NUMBER_OF_AXES;
		static const uint8_t		packet_sensor_buttons = (NUMBER_OF_BUTTONS / 8) + 1;
	...
	struct sensor_data_t {
		...
		//! Axis data
		uint16_t 					packet_sensor_axis_data[NUMBER_OF_AXES];

		//! Button data
		uint16_t 					packet_sensor_button_data[(NUMBER_OF_BUTTONS / 8) + 1];
	}_sensor_data;
