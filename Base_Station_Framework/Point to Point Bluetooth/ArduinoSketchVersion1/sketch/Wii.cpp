                        ...
                        if(l2capinbuf[8] == 0xA1) { // HID_THDR_DATA_INPUT
                                ...
                                switch(l2capinbuf[9]) {
                                        ...
                                        case 0x35: // Core Buttons and Accelerometer with 16 Extension Bytes
                                                ...
                                                }else if(balanceBoardConnected){
                                                	    topRight = (l2capinbuf[15] << 8) | (l2capinbuf[16]);
                                                	    bottomRight = (l2capinbuf[17] << 8) | (l2capinbuf[18]);
                                                	    topLeft = (l2capinbuf[19] << 8) | (l2capinbuf[20]);
                                                	    bottomLeft = (l2capinbuf[21] << 8) | (l2capinbuf[22]);
														valid_packet = true;
                                                }
                                                break;
#ifdef DEBUG_USB_HOST
...
void WII::readMoteCalData() {
        readData(0x0016, 8, true);
}

void WII::readBalCalData() {
        readData(0xA40024, 24, true);
}
