...
void BluetoothConnectionHandler::begin(){
	...
	while(timeout --){
		...
		if(this->_serial->available()){
			...
			//! Connected?
			if(rx_byte == CONNECT){
				this->_utils->connection_state = CONNECTED;
				...
				this->_utils->connection_state = DISCONNECTED;
...
void BluetoothConnectionHandler::_read_packet(){
	...
	this->_serial->readBytes(header_packet, length);
...
void BluetoothConnectionHandler::_get_data(){
	...
	memmove((void*)(this->_packet._heartbeat + sizeof(sensor_packet_header_t)),
...
uint8_t* BluetoothConnectionHandler::_get_packet(uint8_t length){
	...
	//! Get the data
	this->_serial->readBytes(packet, len);
	return packet;
}
