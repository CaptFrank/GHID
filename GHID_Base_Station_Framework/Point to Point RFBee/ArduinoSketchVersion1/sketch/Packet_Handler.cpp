...
void PACKET_HANDLER::poll(void){
	...
	if(EMPTY == available){
		...
		while(available --){
			...
			DEBUG_SERIAL.print(PACKET_PREAMBLE);
			#endif
	}
}
...
void PACKET_HANDLER::_move_state(byte data_byte){
	...
	case PACKET_WAIT_PHASE_2:
		if (PACKET_PREAMBLE == data_byte) {
            _phase++;

            //! prepare for the header
            _data_in = 0;
            _data_expected = sizeof(struct sensor_packet_header_t);
	...
	case PACKET_WAIT_SUM_B:
		if (data_byte == _sum_B) {
            ...
            for (table_index = 0; PACKET_NULL != _handler_table[table_index].packet_id; table_index++)
